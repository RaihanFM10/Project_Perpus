...
<head>
    <meta charset="UTF-8">
    <title>Dashboard Petugas | PerpusInd</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── PAGE HEADER ── */
        .page-hd { display: flex; align-items: flex-start; justify-content: space-between; gap: 14px; flex-wrap: wrap; margin-bottom: 28px; }
        ...
        .ph-date { display: flex; align-items: center; gap: 8px; background: #fff; border: 1px solid var(--gray-200); border-radius: 10px; padding: 9px 15px; font-size: 12.5px; font-weight: 600; color: var(--gray-700); flex-shrink: 0; }
        .ph-date i { color: var(--red); }
        ...
        .sc:hover { transform: translateY(-3px); box-shadow: 0 8px 24px rgba(0,0,0,.07); }

        .sc-top  { display: flex; align-items: center; justify-content: space-between; }
        .sc-icon { width: 42px; height: 42px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 17px; flex-shrink: 0; }
        .sc-badge { font-size: 10px; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; padding: 3px 9px; border-radius: 20px; }

        .sc-lbl { font-size: 12px; font-weight: 600; color: var(--gray-400); margin-bottom: 3px; }
        .sc-val { font-size: 36px; font-weight: 800; color: var(--gray-900); line-height: 1; }

        .sc-foot { display: flex; align-items: center; gap: 6px; font-size: 12px; color: var(--gray-400); padding-top: 12px; border-top: 1px solid var(--gray-100); }
        .sc-foot a { font-weight: 700; font-size: 12px; transition: opacity .15s; }
        .sc-foot a:hover { opacity: .75; }
        ...
        .sec-title::before { content: ''; width: 3px; height: 14px; border-radius: 2px; background: var(--red); flex-shrink: 0; }
        ...
        .qc:hover { transform: translateY(-2px); box-shadow: 0 6px 18px rgba(0,0,0,.07); border-color: rgba(220,38,38,.2); }

        .qc-ico { width: 40px; height: 40px; border-radius: 11px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 16px; }
        .qc-name { font-size: 13px; font-weight: 700; color: var(--gray-900); }
        .qc-desc { font-size: 11.5px; color: var(--gray-400); margin-top: 2px; }
        .qc-arr { margin-left: auto; color: var(--gray-300); font-size: 11px; flex-shrink: 0; transition: color .15s, transform .15s; }
        .qc:hover .qc-arr { color: var(--red); transform: translateX(3px); }

        /* Flag stripe */
        .flag-stripe { display: flex; height: 4px; margin-top: 8px; }
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sb-nav">
        ...
        {{-- <a href="#" class="sb-link"><i class="fas fa-file-download"></i> Generate Laporan</a> --}}
    </nav>
    <div class="sb-footer">
        <div class="sb-user">
...
<main>
    ...
    <div class="stats-grid">
        ...
        <div class="sc au d2">
            <div class="sc-top">
                <div class="sc-icon" style="background:var(--sky-bg);">
                    <i class="fas fa-book" style="color:var(--sky);"></i>
                </div>
                <span class="sc-badge" style="background:var(--sky-bg);color:var(--sky);">Koleksi</span>
            </div>
            <div>
            ...
            <div class="sc-foot">
                <i class="fas fa-arrow-right" style="font-size:9px;color:var(--sky);"></i>
                <a href="{{ url('petugas/books') }}" style="color:var(--sky);">Kelola buku</a>
            </div>
        </div>

        <div class="sc au d3">
            <div class="sc-top">
                <div class="sc-icon" style="background:var(--amber-bg);">
                    <i class="fas fa-hourglass-half" style="color:var(--amber);"></i>
                </div>
                <span class="sc-badge" style="background:var(--amber-bg);color:var(--amber);">Pending</span>
            </div>
            <div>
                <div class="sc-lbl">Menunggu Approval</div>
                <div class="sc-val">{{ $pendingBorrowing }}</div>
            </div>
            <div class="sc-foot">
                <i class="fas fa-arrow-right" style="font-size:9px;color:var(--amber);"></i>
                <a href="{{ route('petugas.borrowings.index') }}" style="color:var(--amber);">Tinjau sekarang</a>
            </div>
        </div>

        <div class="sc au d4">
            <div class="sc-top">
                <div class="sc-icon" style="background:var(--green-bg);">
                    <i class="fas fa-check-circle" style="color:var(--green);"></i>
                </div>
                <span class="sc-badge" style="background:var(--green-bg);color:var(--green);">Approved</span>
            </div>
            <div>
                <div class="sc-lbl">Disetujui</div>
                <div class="sc-val">{{ $approvedBorrowing }}</div>
            </div>
            <div class="sc-foot">
                <i class="fas fa-arrow-right" style="font-size:9px;color:var(--green);"></i>
                <a href="{{ route('petugas.borrowings.index') }}" style="color:var(--green);">Lihat peminjaman</a>
            </div>
        </div>
    ...
    <div class="quick-grid au d6">
        <a href="{{ url('petugas/books') }}" class="qc">
            <div class="qc-ico" style="background:var(--sky-bg);"><i class="fas fa-book" style="color:var(--sky);"></i></div>
            <div>
                <div class="qc-name">Kelola Buku</div>
                <div class="qc-desc">Lihat koleksi buku</div>
            </div>
            <i class="fas fa-chevron-right qc-arr"></i>
        </a>

        <a href="{{ route('petugas.borrowings.index') }}" class="qc">
            <div class="qc-ico" style="background:var(--green-bg);"><i class="fas fa-handshake" style="color:var(--green);"></i></div>
            <div>
                <div class="qc-name">Riwayat Peminjaman</div>
                <div class="qc-desc">Pantau aktivitas pinjam</div>
            </div>
            <i class="fas fa-chevron-right qc-arr"></i>
        </a>

        <a href="{{ route('petugas.borrowings.index') }}?status=pending" class="qc">
            <div class="qc-ico" style="background:var(--amber-bg);"><i class="fas fa-clock" style="color:var(--amber);"></i></div>
            <div>
                <div class="qc-name">Tinjau Pending</div>
                <div class="qc-desc">{{ $pendingBorrowing }} menunggu persetujuan</div>
            </div>
            <i class="fas fa-chevron-right qc-arr"></i>
        </a>
    </div>
